...
public class Enemy_Spawner : MonoBehaviour
{
    ...
    private void Spawn_Check()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, 50, LayerMask.GetMask("Ground")))
        {
            ...
            Instantiate(melee[Random.Range(0,melee.Length-1)], temp_pos + new Vector3(6,0,6), Quaternion.identity);
            Instantiate(ranged[Random.Range(0,ranged.Length-1)], temp_pos, Quaternion.identity);
            Instantiate(tank[Random.Range(0,tank.Length-1)], temp_pos + new Vector3(-6,0,-6), Quaternion.identity);
